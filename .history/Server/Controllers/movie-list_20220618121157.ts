...
{
   let movieArray = Movie.find(function(err, moviesCollection)
    {
        if (err)
        {
            console.error(err);
    ...
    res.render('index', {title: 'Movie List', page: 'movie-list', movies: movieArray, displayName: UserDisplayName(req)});
};
